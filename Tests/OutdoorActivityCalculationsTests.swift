...
final class OutdoorActivityCalculationsTests: XCTestCase {
    ...
    func testDistanceOfASingleKnownSegment() {
        ...
        let points = [
            RoutePoint(latitude: 30.2672, longitude: -97.7431, altitudeMeters: 150, recordedAt: .now),
            RoutePoint(latitude: 30.2682, longitude: -97.7431, altitudeMeters: 150, recordedAt: .now)
        ]
    ...
    func testDistanceOfEmptyOrSinglePointIsZero() {
        ...
        XCTAssertEqual(OutdoorActivityMath.totalDistanceMeters([
            RoutePoint(latitude: 0, longitude: 0, altitudeMeters: 0, recordedAt: .now)
        ]), 0)
    }
    ...
    func testDistanceSumsMultipleSegments() {
        let points = (0...3).map { i in
            RoutePoint(latitude: 30.2672 + Double(i) * 0.001, longitude: -97.7431,
                      altitudeMeters: 150, recordedAt: .now)
        }
        let distance = OutdoorActivityMath.totalDistanceMeters(points)
    ...
    func testElevationGainIgnoresNoiseBelowThreshold() {
        ...
        let points = [
            RoutePoint(latitude: 0, longitude: 0, altitudeMeters: 100, recordedAt: .now),
            RoutePoint(latitude: 0, longitude: 0, altitudeMeters: 101.5, recordedAt: .now),
            RoutePoint(latitude: 0, longitude: 0, altitudeMeters: 100.2, recordedAt: .now),
            RoutePoint(latitude: 0, longitude: 0, altitudeMeters: 101.8, recordedAt: .now)
        ]
    ...
    func testElevationGainCountsRealClimbs() {
        ...
        let points = [
            RoutePoint(latitude: 0, longitude: 0, altitudeMeters: 100, recordedAt: .now),
            RoutePoint(latitude: 0, longitude: 0, altitudeMeters: 110, recordedAt: .now), // +10
            RoutePoint(latitude: 0, longitude: 0, altitudeMeters: 102, recordedAt: .now), // -8, not counted as gain
            RoutePoint(latitude: 0, longitude: 0, altitudeMeters: 107, recordedAt: .now)  // +5
        ]
    ...
    func testElevationGainOfFlatRouteIsZero() {
        let points = (0..<10).map { _ in
            RoutePoint(latitude: 0, longitude: 0, altitudeMeters: 200, recordedAt: .now)
        }
        XCTAssertEqual(OutdoorActivityMath.elevationGainMeters(points), 0)
    }
    ...
    func testCustomThresholdIsRespected() {
        let points = [
            RoutePoint(latitude: 0, longitude: 0, altitudeMeters: 100, recordedAt: .now),
            RoutePoint(latitude: 0, longitude: 0, altitudeMeters: 101, recordedAt: .now)
        ]
